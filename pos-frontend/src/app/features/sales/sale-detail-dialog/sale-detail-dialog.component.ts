...
@Component({
    ...
    <div class="detail-wrap">
      ...
      <div mat-dialog-actions class="detail-actions">
        ...
        @if (sale.status === 'COMPLETED') {
          <button mat-stroked-button class="cancel-btn" (click)="cancelSale()" [disabled]="cancelling">
            @if (cancelling) { <mat-spinner diameter="16" /> } @else { CANCEL SALE }
          </button>
        }
      </div>
    </div>
  ...
  styles: [`
    ...
    h2 { font-size: 20px; font-weight: 700; color: #1a2332; margin: 0; }
    ...
    .meta-row span:last-child { font-size: 13px; font-weight: 600; color: #1a2332; }
    ...
    .total-row.grand { font-weight: 700; font-size: 16px; color: #1a2332; border-top: 1px solid #eee; margin-top: 4px; padding-top: 8px; }
  ...
  cancelSale() {
    ...
    this.saleService.cancel(this.sale.id).subscribe({
      ...
      error: err => {
        ...
        this.snack.open(err.error?.message || 'Cannot cancel', 'OK', { duration: 3000 });
      }
    });
  }
}
